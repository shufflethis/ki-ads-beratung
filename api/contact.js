export default async function handler(req, res) {
    ...
    const blocks = [
        {
            type: 'header',
            text: { type: 'plain_text', text: '📢 Neue Anfrage: KI Ads Beratung', emoji: true }
        },
        {
            type: 'section',
            fields: [
        ...
        },
        {
            type: 'context',
            elements: [
                { type: 'mrkdwn', text: `📍 Quelle: ki-ads-beratung.de | ${new Date().toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })}` }
            ]
        }
    ]
    ...
    try {
        const slackRes = await fetch('https://slack.com/api/chat.postMessage', {
            ...
            body: JSON.stringify({
                channel,
                blocks,
                text: `Neue Anfrage von ${name} (${email}) via ki-ads-beratung.de`,
            }),
        })
